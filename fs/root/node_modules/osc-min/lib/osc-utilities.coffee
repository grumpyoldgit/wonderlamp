...
# written by bambam to parse midi values
exports.splitMidi = (buffer) ->
  channel = binpack["unpackInt8"] buffer[0]
  status = binpack["unpackInt8"] buffer[1]
  data1 = binpack["unpackInt8"] buffer[2]
  data2 = binpack["unpackInt8"] buffer[3]
  return { type: 'midi', channel: channel, status: status, data1: data1, data2: data2}

exports.packMidiObject = (midiObject) ->
  if typeof midiObject isnt "object"
    throw new Error "cannot pack a midi object that isn't an object"
  buf = new Buffer(4)
  buf[0] = (binpack["packInt8"] midiObject.channel)[0]
  buf[1] = (binpack["packInt8"] midiObject.status)[0]
  buf[2] = (binpack["packInt8"] midiObject.data1)[0]
  buf[3] = (binpack["packInt8"] midiObject.data2)[0]
  return buf
# /bambam
...
exports.argToTypeCode = (arg, strict) ->
  ...
  # bambam
  if typeof value is 'object' and value.type == "midi"
    return 'm'
  # /bambam
